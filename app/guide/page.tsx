    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <Link
            href="/"
            className="rounded-lg border border-slate-300 px-3 py-1.5 text-sm font-semibold text-slate-900 hover:border-slate-400"
          >
            Open Builder
          </Link>
        </div>
      ...
      <main className="mx-auto max-w-6xl px-4 py-14 sm:px-6 lg:px-8">
        <section className="grid gap-8 lg:grid-cols-[1.2fr_1fr] items-start">
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-3 text-sm text-slate-600">
              <span className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1">
                <span className="h-2 w-2 rounded-full bg-emerald-500" /> ATS
                safe
              </span>
              <span className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1">
              ...
              <span className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1">
                <span className="h-2 w-2 rounded-full bg-slate-500" /> 1 page
                first
              </span>
            </div>
          </div>
          ...
          <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div
              className="absolute -left-10 top-10 h-28 w-28 rounded-full bg-emerald-100 blur-3xl"
              aria-hidden
            />
            <div
              className="absolute -right-8 bottom-6 h-20 w-20 rounded-full bg-amber-100 blur-2xl"
              aria-hidden
            />
            <div className="relative p-6">
              ...
              <div className="mt-4 space-y-3 text-sm text-slate-700">
                <div className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
        ...
        <section className="mt-14 grid gap-6 lg:grid-cols-2">
          {noteCards.map((card) => (
            <article
              key={card.id}
              className={`group relative overflow-hidden rounded-2xl border ${card.accent} bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md`}
            >
              ...
              <div className="space-y-3 px-5 py-4 text-sm text-slate-700">
                {card.points.map((point, idx) => (
                  ...
                  >
                    <span className="mt-1 inline-flex h-2 w-2 shrink-0 rounded-full bg-slate-400" />
        ...
        <section className="mt-14 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link
              href="/"
              className="inline-flex items-center gap-2 rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-900 hover:border-slate-400"
            >
              Go to Builder
              <svg
                className="h-4 w-4"
